...
public class LightingPresetManage_context : MonoBehaviour
{
    [Header("Main Light")]
    ...
    // Preset이 비어 있으면 기본값을 자동 생성한다.
    private void CreateDefaultPresetsIfEmpty()
    {
    ...
    {
        presets = new LightingPreset[4];

        presets[0] = CreatePreset(
            "Day",
            "#FFF4D6",
            1.2f,
        ...
        presets[1] = CreatePreset(
            "Sunset",
            "#FFB56B",
            0.8f,
            new Vector3(10f, -40f, 0f),
            "#8C6D5F",
            true,
            "#D28A63",
            0.010f,
    ...
    // 기본 프리셋 생성용 함수
    private LightingPreset CreatePreset(
        string name,
        string sunHex,
        float sunIntensity,
        Vector3 sunRotation,
        ...
        float motionBlur)
    {
        ...
        preset.sunColor = HexToColor(sunHex);
        preset.sunIntensity = sunIntensity;
        preset.sunRotation = sunRotation;

        preset.ambientColor = HexToColor(ambientHex);

        preset.useFog = useFog;
        preset.fogColor = HexToColor(fogHex);
        preset.fogDensity = fogDensity;

        preset.nightLightsOn = nightLightsOn;

        preset.ssaoEnabled = true;
        preset.ssaoIntensity = ssaoIntensity;
        preset.ssaoRadius = ssaoRadius;
        preset.ssaoDirectLightingStrength = ssaoDirect;

        preset.bloomIntensity = bloomIntensity;
        preset.bloomThreshold = bloomThreshold;

        preset.postExposure = postExposure;
        preset.contrast = contrast;
        preset.saturation = saturation;
        preset.colorFilter = HexToColor(colorFilterHex);

        preset.vignetteIntensity = vignetteIntensity;
        preset.vignetteSmoothness = vignetteSmoothness;

        preset.chromaticAberrationIntensity = chromatic;

        preset.useDepthOfField = useDof;
        preset.dofStart = dofStart;
        preset.dofEnd = dofEnd;
        preset.dofMaxRadius = dofMaxRadius;

        preset.motionBlurIntensity = motionBlur;
    ...
    private void OnGUI()
    {
        ...
        if (presets == null || presets.Length == 0) return;
        if (currentPresetIndex < 0 || currentPresetIndex >= presets.Length) return;
